    ...
    <div class="p-2 md:p-4">
      <div class="w-full px-6 pb-8 mt-8 sm:max-w-xl sm:rounded-lg">
        ...
        <div class="grid max-w-2xl mx-auto mt-8">
          ...
          >
            <img
              class="w-40 h-40 p-1 rounded-full ring-2 ring-indigo-300 dark:ring-indigo-500 object-center"
              :src="profileImage"
              alt="Bordered avatar"
            />

            <div
              class="flex flex-col items-end justify-end h-full space-y-5 sm:ml-8"
            >
              <!-- <button
                type="button"
                class="py-3.5 px-7 text-base font-medium text-white focus:outline-none bg-primary rounded-lg border border-indigo-200 hover:bg-dark-blue focus:z-10 focus:ring-4 focus:ring-indigo-200"
              >
                Change picture
              </button> -->
              ...
              <input
                ...
                ref="fileupload"
                accept="image/*"
                class="py-3.5 px-7 text-base font-medium text-white focus:outline-none bg-primary rounded-lg border border-indigo-200 hover:bg-dark-blue focus:z-10 focus:ring-4 focus:ring-indigo-200 w-full"
                @change="onSelect"
              />

              <button
                type="button"
                class="py-3.5 px-7 text-base font-medium text-indigo-900 focus:outline-none bg-white rounded-lg border border-indigo-200 hover:bg-indigo-100 hover:text-[#202142] focus:z-10 focus:ring-4 focus:ring-indigo-200 w-full"
                @click="deletePicture"
              >
                Delete
              </button>
...
const onSelect = (event: Event) => {
  ...
  if (input.files && input.files[0]) {
    ...
    const final = new File([blob], file.name, { type: 'image/jpeg' });

    const formData = new FormData();
    formData.append('file', final);

    auth.uploadProfileImage(formData);
  }
};
